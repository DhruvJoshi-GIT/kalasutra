...
| Phase 2: artisan onboarding, products CRUD, uploads (Postgres bytea), artisan orders | ⬜ next |
| AI pipelines (enhance / catalog / price) in fixture mode + jobs + polling UI | ⬜ |
| Deploy: Render (API) + Neon (DB) via `render.yaml`; keep-warm ping | ⬜ user to create accounts |
| Domain kalasutra.live -> GitHub Pages | ⬜ CNAME + Pages set; **DNS at Name.com pending** |
| Sarvam key in `backend/.env` | ✅ pasted by the user · Anthropic / Voyage / fal keys ⬜ |
...
cd D:\swadesh\backend; .venv\Scripts\python -m uvicorn app.main:app --port 8000
